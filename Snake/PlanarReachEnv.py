...
from .planar import PlanarSnake
...
@register_env("PlanarReach-v1", max_episode_steps=50)
class PlanarReachEnv(BaseEnv):
    SUPPORTED_REWARD_MODES = ["dense"]
    agent: PlanarSnake
    goal_thresh = 0.025

    def __init__(self, *args, robot_uids="planar_snake", **kwargs):
        super().__init__(*args, robot_uids=robot_uids, **kwargs)

    @property
    def _default_sensor_configs(self):
        pose = sapien_utils.look_at([0, 0, 1.5], [0.0, 0.0, 0.2])
        return [CameraConfig("base_camera", pose, 128, 128, np.pi / 2, 0.01, 100)]

    @property
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([0, 0, 3], [0.0, 0.0, 0.2])
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            b = len(env_idx)
            min_dist = 0.4 * self.agent.height
